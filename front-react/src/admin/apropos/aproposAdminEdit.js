...
function AproposAdminEdit() {
  ...
  const apiAproposUpdate = `http://localhost:8000/api/aproposs/${id}`;
  ...
  const getApropos = async (e) => {
    await axios
      .get(apiAproposUpdate,
        {
          headers: { 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setIsLoading(true);
      const response = await fetch(apiAproposUpdate, {
          method: 'PUT',
          headers: {
              'Content-Type': 'application/ld+json'
          },
...
export default AproposAdminEdit;
